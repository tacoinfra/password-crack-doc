...
1. your password uses a combination of 90 characters including A-Z, a-z, 0-9, and symbols from a typical keyboard (!, $, %, &, +, etc.).
...
Every additional character exponentially increases the number of password possibilities:
...
You will **never** recover a long random password set by your browser or a generation tool.
...
> Note: you can run password recovery tools in Windows WSL2, a Virtual Machine, or Docker container, but these are not the fastest options and require additional configuration to access GPUs. This tutorial recommends a bare-metal installation.
...
1. [start John the Ripper](06_start-jtr.md) using various recovery strategies.
